...
function findIndexByProperty(arr, name) {
  ...
  const arrFoundIndex = arr.findIndex(({ name }) => name === "Petro");

  return arrFoundIndex;
}
...
function findLastEvenNumber(arr) {
  ...
  let arrLast = arr.findLast((element) => element % 2 == 0);
...
function getSubArrayAndConvertToString(arr, startIdx) {
  ...
  const arrJoin = arr.slice(3);
  const arrSlice = arrJoin.join(" ");

  return arrSlice.toString();
}
...
function getPositiveArrayValues(arr) {
  ...
  const arrayValueIterator = arr.values();
  const arrayValues = Array.from(arrayValueIterator);
  ...
  //     return [value];
  //   }
  //   const arrFilter = arr.filter(iterator > 0);
...
function removeAndDouble(arr, startIdx) {
  ...
  const arrSmall = arr.splice(0, startIdx);

  const doubleArr = arrSmall.map((x) => x * 2);
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  const doubleArr = arrFlat.map((x) => x ** 2);
  // За допомогою методу filter() фільтруємо елементи, які більше за порогове значення.
  const positiveArrayValues = doubleArr.filter((value) => value > threshold);
...
function wordLengths(arr) {
  ...
  const lengthsArray = arr.map((element) => (element = element.length));
...
function insertElementAt(arr, index, element) {
  ...
  const arrSplice = arr.splice(index, 0, element);
...
function flattenArray(arr) {
  ...
  const flattenedArray = arr.flat(1 / 0);
...
function findLastNegativeNumberIndex(arr) {
  ...
  let lastIndex = arr.findLastIndex((element) => (element = -element));
